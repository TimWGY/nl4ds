...
def show_map(data, tile_style='bw'):
  ...
  for _, row in data.iterrows():
    if 'place_type' in data.columns:
      ...
      folium.Circle(row['coordinates'], radius=1, color='red' if row['CHINESE'] else 'beaver', tooltip=f"Name: {row['Name']}<br>Address: {row['Address']}<br>Year: {int(row['YR'])}<br>ID: {row['UUID']}").add_to(m)
